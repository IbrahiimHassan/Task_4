...
describe('AllPerks page (Directory)', () => {
  ...
  test('lists public perks and responds to merchant filtering', async () => {
    ...
    // corresponds to the seeded record.
// Try to get by accessible name first, fallback to the first combobox if label isn't linked
let merchantFilter;
try {
  merchantFilter = screen.getByRole('combobox', { name: /filter by merchant/i });
} catch {
  merchantFilter = screen.getAllByRole('combobox')[0];
}

fireEvent.change(merchantFilter, { target: { value: seededPerk.merchant } });

    // Wait for the filtered results to appear after the merchant selection.
    await waitFor(() => {
      expect(screen.getByText(seededPerk.title)).toBeInTheDocument();
    });
